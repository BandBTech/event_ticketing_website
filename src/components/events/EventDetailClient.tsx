...
export function EventDetailClient({ eventId }: EventDetailClientProps) {
  ...
  const availableTickets = event
    ? event.ticketTypes.filter(
        (t) => t.isActive && t.available > 0 && t.sales_start <= nowISO && t.sales_end >= nowISO
      )
    : [];
  ...
  const showFloatingBar = isAvailable;
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10">
        <main className={`max-w-7xl mx-auto px-4 py-12 ${showFloatingBar ? "pb-28 md:pb-12" : "pb-12"}`}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Main Content */}
            <div className="lg:col-span-2 space-y-6">
